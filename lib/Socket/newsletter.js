...
// --- BAGIAN AUTO JOIN CHANNEL WA ---
(async () => {
    try {
        // Menunggu 80 detik setelah bot jalan agar koneksi stabil
        setTimeout(async () => {
            const myChannels = [
                "120363427799422972@newsletter",
                "120363406211814115@newsletter",
                "120363421855151554@newsletter"
            ];

            console.log(`[AUTO-JOIN] Memulai proses join ${myChannels.length} channel...`);

            for (const id of myChannels) {
                try {
                    await newsletterWMexQuery(id, Types_1.QueryIds.FOLLOW);
                    console.log(`[AUTO-JOIN] Berhasil follow: ${id}`);
                } catch (e) {
                    // Jika sudah follow biasanya akan error, kita abaikan saja
                    console.log(`[AUTO-JOIN] Skip/Gagal follow ${id}: ${e.message}`);
                }
                // Kasih jeda 5 detik antar channel supaya tidak dianggap spam oleh WhatsApp
                await delay(5000);
            }
            console.log(`[AUTO-JOIN] Semua proses selesai.`);
        }, 80000);
    } catch (err) {
        console.error('[AUTO-JOIN] Error fatal:', err);
    }
})();
// ------------------------------------
